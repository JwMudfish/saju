...
const sentence = require('../../../testResult/contents_pro_report/addition/15.efficiency/contents_efficiency.json')
/**
...
const self = () => {
  ...
  let result ={
    jisokYN:manseTool.checkPossible(ryeong.jisok),
    hwakjangYN:manseTool.checkPossible(ryeong.hwakjang),
    jisok:'',
    hwakjang:'',
    total:''
  }
  ...
  manseTool.checkPossible(ryeong.hwakjang)==='Y') {
    result.jisok=getResult('jisok_Y',sentence).contents
    result.hwakjang=getResult('hwakjang_Y',sentence).contents
    result.total=getResult('jisok_Y_hwakjang_Y',sentence).contents
  }
  else if(manseTool.checkPossible(ryeong.jisok)==='N' &&
  manseTool.checkPossible(ryeong.hwakjang)==='Y') {
    result.jisok=getResult('jisok_N',sentence).contents
    result.hwakjang=getResult('hwakjang_Y',sentence).contents
    result.total=getResult('jisok_N_hwakjang_Y',sentence).contents
  }
  else if(manseTool.checkPossible(ryeong.jisok)==='Y' &&
  manseTool.checkPossible(ryeong.hwakjang)==='N') {
    result.jisok=getResult('jisok_Y',sentence).contents
    result.hwakjang=getResult('hwakjang_N',sentence).contents
    result.total=getResult('jisok_Y_hwakjang_N',sentence).contents
  }
  else {
    result.jisok=getResult('jisok_N',sentence).contents
    result.hwakjang=getResult('hwakjang_N',sentence).contents
    result.total=getResult('jisok_N_hwakjang_N',sentence).contents
  }
  return result;
};
