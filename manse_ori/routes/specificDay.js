...
const setInfo = require("../manse/manseFunc/specialDaySetInfo");
const resultFunc = require("../manse/manseFunc/spaciaDayFunc");
    ...
    jwt.verify(token, secretKey, function (err, decoded) {
      if (err) {
        ...
        if (user[userNum].auth.includes(contents)) {
          ...
          resultFunc.resultFunc(res, 'specialDay', contents);
        } else {
          let errorMessage = {
            message: 'Error 접근권한이 없습니다',
            errorCode: 'NoAuth',
