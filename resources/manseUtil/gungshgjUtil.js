 ...
 */
exports.makeElementArray = (func, shgj) => {
  const pillar = [
    usePillar.y_sky,
    usePillar.m_sky,
    usePillar.d_sky,
  ...
  const yuksin = [
    ...
    useYuksin.y_jangan.m_jangan1,
    useYuksin.y_jangan.m_jangan2,
    useYuksin.y_jangan.m_jangan3,
  ...
  for (let i = 0; i < pillar.length; i++) {
    if (shgj !== undefined) {
      result.push(func(used[i], sangguk[i], pillar[i], yuksin[i]))
    }
    else {
      result.push(func(used[i], pillar[i], yuksin[i]))
    }
