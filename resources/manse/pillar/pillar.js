...
pillarFunc.pillar = function () {
  return new Promise((resolve) => {
    let resultMonth = Number(useDate.month);
    let resultTime = useDate.hour;
    let resultMinute = useDate.minute;
    ...
    if (myBirth.diff(moment(useJulib.julib), "minutes") <= 0) {
      ...
      //myManse(최종본)에다가 담는다.
      myManse.julib = useJulib.beforeJulib;
      myManse.julibGanji = useJulib.beforeJulibGanji;
      myManse.junggiGanji = useJulib.beforeJunggiGanji;
      myManse.junggi = useJulib.beforeJunggi;
      useNextJulib = useJulib.julib;
    } else {
      //그외경우
      //이건 건들ㄴㄴ
      compareJulib = moment(useJulib.nextJulib, "YYYY-MM-DD HH:mm:ss ");
      //중기보다 작으면
      if (myBirth.diff(moment(useJulib.junggi), "minutes") <= 0) {
        //myManse(최종본)에다가 담는다.
        myManse.julib = useJulib.julib;
        myManse.julibGanji = useJulib.julibGanji;
        myManse.junggi = useJulib.junggi;
        myManse.junggiGanji = useJulib.junggiGanji;
        useNextJulib = useJulib.nextJulib;
      } else {
        //myManse(최종본)에다가 담는다.
        myManse.julib = useJulib.julib;
        myManse.julibGanji = useJulib.julibGanji;
    ...
    useRealJulib.julib=myManse.julib 
    useRealJulib.julibGanji=myManse.julibGanji 
    useRealJulib.junggi=myManse.junggi 
    useRealJulib.junggiGanji=myManse.junggiGanji 
    ...
    if (
      ...
      yearPillar = year.getYear(Number(useDate.year) - 1);
    } else {
      yearPillar = year.getYear(useDate.year);
    }
    ...
    myManse.pillar = pillar;
    usePillar = pillar;
    useSolar = useSolar + pillar.h_land + "시";
    useLunar = useLunar + pillar.h_land + "시";
