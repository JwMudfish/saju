...
summerSeoul.summerSeoul = function () {
  ...
  if (
    Number(useSpecialDate.summer_time) === 1 &&
    Number(useSpecialDate.seoul_time) === 30
  ) {
    ...
    Number(useSpecialDate.summer_time) === 1 &&
    Number(useSpecialDate.seoul_time) === 0
  ) {
    if (summerTF === false) {
      result = {
        errorMessage: {
    ...
    Number(useSpecialDate.summer_time) === 0 &&
    Number(useSpecialDate.seoul_time) === 30
  ) {
    if (seoulTF === false) {
      result = {
...
function checkSeoulTime() {
  ...
  let myBirth = moment(
    useSpecialDate.year +
      "-" +
      useSpecialDate.month +
      "-" +
      useSpecialDate.day +
      " " +
      useSpecialDate.hour +
      ":" +
      useSpecialDate.minute +
      ":00",
    "YYYY-MM-DD HH:mm:ss "
  );
  if (
