...
umYangOHangFunc.umYangOHang = function () {
  return new Promise((resolve) => {
    //umYangFunc.umYang(usePillar.y_sky,1) ->1이면 천간 2면 지지
    //천간신금인지 지지신금인지 구분하기위한 용도!
    //천간
    let ySky = {
      umYang: umYangFunc.umYang(usePillar.y_sky, 1),
      oHang: oHangFunc.oHang(usePillar.y_sky),
    };
    let mSky = {
      umYang: umYangFunc.umYang(usePillar.m_sky, 1),
      oHang: oHangFunc.oHang(usePillar.m_sky),
    };
    let dSky = {
      umYang: umYangFunc.umYang(usePillar.d_sky, 1),
      oHang: oHangFunc.oHang(usePillar.d_sky),
    };
    let hSky = {
      umYang: umYangFunc.umYang(usePillar.h_sky, 1),
      oHang: oHangFunc.oHang(usePillar.h_sky),
    };
    //지지
    let yLand = {
      umYang: umYangFunc.umYang(usePillar.y_land, 2),
      oHang: oHangFunc.oHang(usePillar.y_land),
    };
    let mLand = {
      umYang: umYangFunc.umYang(usePillar.m_land, 2),
      oHang: oHangFunc.oHang(usePillar.m_land),
    };
    let dLand = {
      umYang: umYangFunc.umYang(usePillar.d_land, 2),
      oHang: oHangFunc.oHang(usePillar.d_land),
    };
    let hLand = {
      umYang: umYangFunc.umYang(usePillar.h_land, 2),
      oHang: oHangFunc.oHang(usePillar.h_land),
    };
    let y_jang = usejijanggan.y_jangan;
    let m_jang = usejijanggan.m_jangan;
    let d_jang = usejijanggan.d_jangan;
    let h_jang = usejijanggan.h_jangan;
    ...
    useUmYangOHang = umYangOHang;
    myManse.umYangOHang = umYangOHang;
    resolve(umYangOHang);
  }).catch((error) => {
    console.log(error);
    return error;
